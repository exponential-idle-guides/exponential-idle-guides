...
According to Lemma 1.1, $u$ and $u'$ share the same exponent $r$, let's name the $K$ for the two variables $K$ and $K'$ respectively. Since $b>1$, $K'< K$, from Lemma 1.1 we have:
...
$$v_{mk+l} < mp\frac{q}{q-1}\left(q^\frac{1}{m}\right)^{mk+l}\qquad (1-6)$$
...
$$mpq^{k-1}\left(1+\frac{l}{m}q\right) \ge mpq^{k-1}(1+q)^{\frac{l}{m}}>mpq^{k-1}q^{\frac{l}{m}}$$

$$= \frac{mp}{q}\left(q^{\frac{1}{m}}\right)^{mk+l}$$

Therefore,

$$v_{mk+l} > \frac{mp}{q}\left(q^{\frac{1}{m}}\right)^{mk+l}\qquad (1-8)$$

From (1-6) and (1-8) we have:

$$\frac{mp}{q}\left(q^{\frac{1}{m}}\right)^{mk+l} < v_{mk+l} < mp\frac{q}{q-1}\left(q^\frac{1}{m}\right)^{mk+l}$$

Since $mk+l = L$ in this case,

$$\frac{mp}{q}\left(q^{\frac{1}{m}}\right)^L < v_L < mp\frac{q}{q-1}\left(q^\frac{1}{m}\right)^L$$

Apparently, $\left(\frac{mp}{q},\frac{mpq}{q-1}\right) \in \mathbb{R}^+$ and $q^\frac{1}{m} \in \mathbb{R}^+$. Therefore, there exist $(c_1,c_2)$ and $d$ that fit the conditions.
...
In which $r \in \mathbb{R}^+$. From (1-9), (1-10) and the definition mentioned in , we have:
...
1. Multiplication:  $\Theta(\rho^{r_1}) \cdot \Theta(\rho^{r_2}) \rightarrow \Theta\left[\rho^{(r_1+r_2)}\right]$
2. Addition:  $\Theta(\rho^{r_1}) + \Theta(\rho^{r_2}) \rightarrow \Theta\left[\rho^{max(r_1,r_2)}\right]$
...
As $\rho$ approaches infinity, we can see that $q$ approaches $\sqrt{16q_1q_2t}$. We can then do the asymptotic analysis on the entire system normally from here.
...
In an Endgame situation, $t \rightarrow \infty$, therefore, $C$ is ignorable.
...
$$\frac{\ln(\rho) - \ln(\rho_{pub})}{t} = \frac{\ln\left[K'(1-x)\rho^{z}_{pub}t^{y+1}\right]^{\frac{1}{1-x}} - \ln(\rho_{pub})}{t}$$

$$ = \frac{\frac{\ln\left[K'(1-x)\right]+\ln\left(\rho^{z}_{pub}\right)+\ln\left(t^{y+1}\right)}{1-x} - \ln(\rho_{pub})}{t}\qquad (3-5)$$

In the Endgame, $\ln\left[K'(1-x)\right]$ is ignorable:

$$\frac{\frac{\ln\left[K'(1-x)\right]+\ln\left(\rho^{z}_{pub}\right)+\ln\left(t^{y+1}\right)}{1-x} - \ln(\rho_{pub})}{t} \simeq \frac{\frac{\ln\left(\rho^{z}_{pub}\right)+\ln\left(t^{y+1}\right)}{1-x} - \ln(\rho_{pub})}{t}$$
...
$$\frac{d\rho}{dt} = q_1q_2\left(c_1^{1.15}c_2 + c_3\rho^{0.2} + c_4\rho^{0.3}\right)$$
...
In which $c_n$ and $d_n$ are purchasable variables. Using Lemma 1.5, we can get:
