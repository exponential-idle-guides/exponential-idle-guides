...
While this model is great, it has one flaw to analyze official CTs, as it assumes $\Pi = \rho^{z}_{pub}$, or in other words that $\frac{d\rho}{dt} \propto \Pi$, which is not the case for every CT.
Let's decompose $z = ab$, where $a$ is such that $\Pi \propto \rho^{a}$. In this case, we have $\frac{d\rho}{dt} \propto \Pi^b$ or $\Pi^{b} = \rho^{z}_{pub}$, a more general model.
...
Note that in practice, due to $z + x$ being less than 1 along with other factors, the optimal multiplier fluctuates and is generally lower than that.
...
$$\dot{\rho}=K\Pi q \Rightarrow \dot{\rho}=K\Pi t$$
