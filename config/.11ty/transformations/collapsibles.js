...
module.exports = function (config, exclusions) {
  config.addTransform("collapsibles", function(content, outputPath) {
    if (!exclusions.includes(outputPath) 
      ...
      const collap_dict = copy_dict(h_dict, (d, h) => Object.keys(d[h]).reduce((a, b) => ({...a, [b] : d[h][b].filter((i, idx) => b === "ids" ? !(i === '#' || skipped(skiplist, $(i).attr('class'), i)) : a.ids.includes(d[h].ids[idx]))}), {}));
      const sidebar_collap_dict = copy_dict(sidebar_h_dict, (d, h) => Object.keys(d[h]).reduce((a, b) => ({...a, [b] : d[h][b].filter((i, idx) => b === "ids" ? !(i === '#' || skipped(skiplist, $(i).attr('class'), i)) : a.ids.includes(d[h].ids[idx]))}), {}));
